...
script_path = 'C:\\Users\\LY-OuYang\\Desktop\\appserver.py'
...
# 日志文件
log_file_path = 'E:\\wiki\\8_Python\\check_processes\\log\\monitor.log'
# python执行exe目录
...
def kill_python_script(process_name):
    ...
    if process_name in output:
        # 使用 taskkill 命令杀死进程
        command = f'taskkill /F /IM python.exe /FI "IMAGENAME eq {process_name}.py"'
        subprocess.call(command, shell=True)
        logger.info(f"The process '{process_name}.py' has been terminated.")
    else:
        logger.info(f"No process with the name '{process_name}.py' is currently running.")
...
def start_script(script_path,script_name):
    # 构建命令行参数
    command = [python_local_path, script_path + script_name]
    try:
        # 后台运行
