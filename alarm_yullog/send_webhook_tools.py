'''
Author       : OuYang
Date         : 2023-12-28 17:44:39
Version      : v1.0
LastEditors: OuYang
LastEditTime: 2024-01-05 15:12:20
Description  : 封装webhook所有消息类型，可用于发送webhook消息
'''
...
class WechatBot:
    ...
    log_file = "E:\\wiki\\8_Python\\send_webhook\\nohup.log"
    log_format = "[{time:YYYY-MM-DD HH:mm:ss.SSS}] - [{level}] - {message}"
    logger.add(log_file, rotation="1 day", format=log_format, level="INFO")
    ...
    def send_text(self, content, mentioned_list=[], mentioned_mobile_list=[]):
        ...
        if response.json().get("errcode") == 0:
            logger.info(f"WebHook send text message succeeded.")
            logger.debug(f"WebHook send text message succeeded. Result: {response.json()}")
            return True
        else:
            logger.error(f"WebHook failed to send text message.{response.text}")
            return False
    ...
    def send_markdown(self, content):
        ...
        if response.json().get("errcode") == 0:
            # info
            logger.info(f"WebHook send markdown message succeeded.")
            # debug
            logger.debug(f"WebHook send markdown message succeeded. Result: {response.json()}")
            return True
        else:
            logger.error(f"WebHook failed to send markdown message.{response.text}")
            logger.debug(f"WebHook failed to send markdown message. Result: {response.json()}")
            return False
